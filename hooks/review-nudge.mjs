#!/usr/bin/env node
// UserPromptSubmit hook for Recall — the active mid-session review ("nudge").
//
...
// agent delegates a memory review to the recall-worker (Haiku) subagent.
//
...
function loadReviewConfig() {
  try {
    const raw = readFileSync(join(homedir(), ".claude", "recall", "recall.config.jsonc"), "utf8");
...
const stateDir = join(homedir(), ".claude", "recall", "state");
...
if (turns >= review.everyNTurns) {
  ...
  const writeRule = review.approvalGate
    ...
    : "Save it directly with recall_save.";
  nudge =
    "<RECALL_REVIEW>\n" +
    ...
    `If yes, delegate to the recall-worker subagent (Haiku) to draft it in Recall notation, then ${writeRule}\n` +
